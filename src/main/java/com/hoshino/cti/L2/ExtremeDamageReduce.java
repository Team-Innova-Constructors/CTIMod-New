...
public class ExtremeDamageReduce extends LegendaryTrait {
    ...
    public void onHurtByOthers(int level, LivingEntity entity, LivingHurtEvent event) {
        if (event.getSource().isBypassArmor()||event.getSource().isBypassMagic()) event.setAmount(event.getAmount()*0.05f);
    }

    @Override
    public boolean allow(LivingEntity le, int difficulty, int maxModLv) {
        ResourceLocation location = ForgeRegistries.ENTITY_TYPES.getKey(le.getType());
        return location!=null&& (location.getNamespace().equals("cataclysm")||location.getNamespace().equals("aquamirae"));
    }
}
